...
using System;
/*
public class BatUpgrade2 : MonoBehaviour
{
    ...
    [System.NonSerialized]
    public bool ulepszenieKnockback = false;
    ...
    private void Start()
    {
        ...
        lifePool = LevelManager.lifePoolBat2;
        lifePoolStala = lifePool;

        //Debug.Log("LIFFFFFFFFE POOOOL" + lifePool);

        float maxMoveSpeed = UnityEngine.Random.Range(0.8f, 1.6f);
    ...
    public void DropCoinow(int maxChance)
    {
        ...
        {  
                ...
                tak.x += (float)i / 2 ;
                tak.y += (float)i / 2;
                var coin = Instantiate(levelManager.coinx3Prefab, tak, gameObject.transform.rotation);
    ...
    public void Death()
    {
        if (lifePool <= 0)
        {
          ...
          ///  Debug.Log("ZNISZCZONO!");
            ...
            chanceToDropCoin = 10;
            chanceToDropHeart = 15;

            levelManager.ExpGain("Bat2");


            DropCoinow(chanceToDropCoin);
            DropSerc(chanceToDropHeart);
           // itemsManager.ItemDrop(gameObject.transform.position, 250f * levelManager.itemDropChanceMultiplier);
            itemsManager.GemDrop(gameObject.transform.position, 250f * levelManager.itemDropChanceMultiplier);
